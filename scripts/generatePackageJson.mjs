...
const pathtoPackage = join(packagesDir, "/");
const pathToFile = join(packagesDir, "package.json");
try {
  ...
  // If the package folder name starts with `ui-` remove this from the npm published package
  // name.
  let publishName = json.name.split(`${scope}-`)[1];
  if (publishName.startsWith("ui-")) {
    // remove "ui-"" from the start of `publishName`.
    publishName = publishName.slice("ui-".length);
  }
